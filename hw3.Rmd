...
a. Use group_by and summarize to show the number of tweets they each politician wrote.
...
```
What are the top three most frequent words? Explain why this result is expected based on Zipf's law. Answer: 
...
Q8. Using the similarity matrix, find the 10 most similar politician-year pairs. Make sure to exclude any self-similarities (e.g. Bernie Sanders is the most similar politician to Bernie Sanders). Print each pair and the similarity score. Make sure to ignore any entries on the diagonal.
...
predict(model, keyword, type = "nearest", top_n = 20)
```
...
Review the code carefully then add arguments to `summarize_if` to take the sum of all numeric columns in `doc.props`.
...
Q. 17 (OPTIONAL EXTRA CREDIT). Use the`estimateEffect` function covered in lecture to analyze the relationship between the covariate and the topic. 
...
Q.18 (OPTIONAL EXTRA CREDIT) Select a topic of interest and plot the differences in content by partisanship. Answer the question below.
